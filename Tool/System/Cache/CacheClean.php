...
/**
 * MCP write tool `system.cache.clean` — invalidate cached data for the
 * given cache types. Mirror of `bin/magento cache:clean`.
 */
class CacheClean implements ToolInterface, UnderlyingAclAwareInterface
{
    public const TOOL_NAME = 'system.cache.clean';
    public const ACL_RESOURCE = 'Magebit_Mcp::tool_system_cache_clean';
    ...
    {
        return 'Invalidate cached data for the given cache types — '
            . 'mirror of `bin/magento cache:clean`. Provide either '
    ...
    public function getInputSchema(): array
    {
        return Schema::object()
            ->array('cache_type', fn (ArrayBuilder $a) => $a
                ...
                ->minItems(1)
                ->description('Cache type ids to clean (e.g. `["config"]`).')
            )
            ->boolean('all', fn (BooleanBuilder $b) => $b
                ->description('Clean every cache type. Mutually exclusive '
                    . 'with `cache_type`.')
            )
            ->toArray();
    }
    ...
    public function execute(array $arguments): ToolResultInterface
    {
        ...
        return new ToolResult(
            ...
            auditSummary: ['cleaned_count' => count($types)]
        );
    }
}
