...
/**
 * MCP write tool `system.cache.enable` — turn on the given cache types.
 * Mirror of `bin/magento cache:enable`.
 */
class CacheEnable implements ToolInterface, UnderlyingAclAwareInterface
{
    public const TOOL_NAME = 'system.cache.enable';
    public const ACL_RESOURCE = 'Magebit_Mcp::tool_system_cache_enable';
    ...
    {
        return 'Enable the given cache types — mirror of '
            . '`bin/magento cache:enable`. Returns the subset whose '
    ...
    public function getInputSchema(): array
    {
        return Schema::object()
            ->array('cache_type', fn (ArrayBuilder $a) => $a
                ...
                ->minItems(1)
                ->description('Cache type ids to enable.')
            )
            ->boolean('all', fn (BooleanBuilder $b) => $b
                ->description('Enable every cache type. Mutually exclusive '
                    . 'with `cache_type`.')
            )
            ->toArray();
    }
    ...
    public function execute(array $arguments): ToolResultInterface
    {
        ...
        $changed = $this->cacheManager->setEnabled($types, true);
